...
class Ball(GameObject):
    ...
    def changeDirection(self, gameObject):
        ...
        if position[1] > objectPosition[1] and position[1] < objectPosition[1] + objectSize[1] and \
            position[0] > objectPosition[0] and position[0] < objectPosition[0] + objectSize[0]:
    ...
    def updatePosition(self):
        ...
        if not self.inMotion():
            padPostition = self.__game.getPad().getPosition()
            self.setPosition((
                padPostition[0] + (GameConstants.PAD_SIZE[0] / 2),
                padPostition[1] - GameConstants.BALL_SIZE[1]
            ))
            return
